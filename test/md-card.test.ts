...
describe('buildMarkdownCard final reply actions', () => {
  it('renders JSON 2.0 callback buttons before the footer', () => {
    const card = JSON.parse(buildMarkdownCard(
      '还没 push。',
      'ou_owner',
      undefined,
      'zh',
    ...
    const rows = card.body.elements.filter((element: any) => element.tag === 'column_set');

    expect(rows).toHaveLength(1);
    expect(rows[0].columns[0].elements[0]).toMatchObject({
      tag: 'button',
      text: { tag: 'plain_text', content: '执行 push' },
      behaviors: [{
        type: 'callback',
        value: {
    ...
    const footerIndex = card.body.elements.findIndex((element: any) =>
      element.text_size === 'notation_small_v2');
    expect(card.body.elements.indexOf(rows[0])).toBeLessThan(footerIndex);
  });
});
