...
describe('codex-app-runner app-server protocol integration', () => {
  ...
  it('fails closed when a grown group receives a NON-canonical completion and history has only a root-only turn (R3-B3)', async () => {
    ...
    try {
      ...
      // 以 runner 已接收原生 progress 为根 turn 可 steer 的证据；请求日志写入
      // 早于 app-server 响应，不能作为并发场景的稳定就绪边界。
      await waitFor(harness, () => control.activities.some(activity =>
        activity.phase === 'progress' && activity.turnId === 'turn-fake-1'), 25_000);
      send('follow', 'om_nc_follow');

      // Two finals expand (group settled fail-closed), and the settlement is a
      // diagnostic conflict — never the foreign root-only answer. 全量并发下
      // 还包含一次有界 history round-trip，沿用同组用例的宽限时间。
      await waitFor(harness, () => control.finals.length === 2
        && control.states.filter(state => state.busy === false).length >= 2, 25_000);
