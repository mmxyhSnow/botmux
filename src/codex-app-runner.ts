...
interface ActiveTurn {
  ...
  /** 已观察到的终态 `turn/completed` 屏障。它可能属于 canonical turn，也可能
   * 是需要通过 bounded history 归因的 non-canonical completion。 */
...
function emitFinalMarker(
  ...
  if (!superseded && usageKey) {
    // 原生 turn 的最终答复已确定后，非阻塞通知 daemon 关闭该 turn 累积的 ask flow。
    dispatchCodexAppUserInputFlowCompletion(appTurnId, userInputContext, writeLine);
  }
...
async function tryAdmitSteer(): Promise<void> {
  ...
  if (turn.completionSeen && turn.terminalCompletion && !turn.startResponsePending) {
    const completedId = typeof turn.terminalCompletion.id === 'string'
      ? turn.terminalCompletion.id
      : undefined;
    if (!completedId || completedId === turn.canonicalNativeTurnId) {
      settleSteeredCompletion(turn, turn.terminalCompletion);
    } else {
      // steer 响应与 non-canonical completion 同批到达时，completion 先被
      // 缓存；成员正式入组后再执行 group-aware history 归因，不能永久挂起。
      void reconcileSteeredGroupFromHistory(turn, completedId);
    }
    return;
  }
...
async function runTurn(message: QueuedInput): Promise<void> {
  ...
  const bufferedTerminalId = typeof turn.terminalCompletion?.id === 'string'
    ? turn.terminalCompletion.id
    : undefined;
  if (!turn.completed
      && turn.completionSeen
      ...
      && !turn.steerInFlight
      && bufferedTerminalId
      && bufferedTerminalId !== turn.canonicalNativeTurnId
      && (turn.accepted?.length ?? 0) > 1) {
    // 若 start 响应是最后解除的屏障，此处接续先前缓存的 non-canonical
    // completion；只通过完整成员序列的 bounded history 结果完成归因。
    void reconcileSteeredGroupFromHistory(turn, bufferedTerminalId);
  }
  ...
  if (pendingCompletions.length > 0 && !turn.completed) {
    ...
    if (exactMatches.length === 1) {
      ...
      // start 响应与不匹配 completion 同批到达时，completion 会先进入缓存。
      // 响应绑定 native id 后必须复用实时路径的 bounded-history 归因，不能因
      // 两条 JSON-RPC 记录落在同一 stdout chunk 就遗漏结算并永久挂起。
      const observedId = [...pendingCompletions].reverse().find(
        completion => typeof completion?.id === 'string',
      )?.id;
...
async function main(): Promise<void> {
  ...
  await controlReady;
  const version = detectedCodexVersion();
  client = new AppServerClient(
    args.codexBin,
    args.cwd,
    !!version && codexVersionAtLeast(version, 0, 146, 0),
  );
  ...
  // 先安装输入监听，再发布已连接和可接收输入的状态；否则控制端看到
  // ready 后立即写入的首条消息可能落在 stdin 监听建立前的竞态窗口。
