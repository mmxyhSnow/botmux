...
async function handleDocCommentAdmitted(ctx: DocCommentContext): Promise<boolean> {
  ...
  try {
    ...
    return await runClaimedDocCommentTurn(claimKey, async () => {
      ...
      if (ds.worker && !ds.worker.killed) {
        // 并发评论可能已由同一路由的另一轮完成唯一一次 refork。此时当前轮
        // 改按 live 形态进入新 worker，不能把可重路由的竞态误报为投递失败。
        const { cliInput: liveInput } = buildDocCommentTurnInput({
          ds,
          promptInput,
          botCliId: dsBotCfg.cliId,
          botCliPathOverride: dsBotCfg.cliPathOverride,
        ...
        rememberLastCliInput(ds, promptContent, liveInput);
        if (!sendWorkerInput(ds, liveInput, turnId)) {
          throw new Error('worker became unavailable during comment:refork-handoff');
        }
        logger.info(`[${tag(ds)}] concurrent doc-comment handed to refork winner (turn ${turnId.slice(0, 8)})`);
        return;
      }
      sessionStore.updateSession(ds.session);
      if (ds.adoptedFrom) {
