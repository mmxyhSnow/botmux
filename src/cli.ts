...
function ecosystemConfig(
  ...
  const baseApp = {
    ...
    interpreter: resolvePm2NodeInterpreter(interpreter),
    cwd: CONFIG_DIR,
    autorestart: true,
    max_restarts: 10,
...
function cmdStatus(): void {
  warnIfLegacyBotmuxAlive();
  console.log(`Live: ${resolveLiveIdentity().display}\n`);
  runPm2(['status']);
}
...
命令:
  ...
  restart     重启 daemon（默认不停止插件 service，core 启动后确保 mode=auto 正在运行；--with-plugin 显式先停再启动 auto service）
              自动识别托管 AI 轮次；--reason <原因>、--source <cli|ai|dashboard> 可显式声明通知归因
  ...
  status      查看 daemon 状态
  worktree    检查版本化运行目录（doctor；clean --unused 默认 dry-run，--apply 才清理）
    ...
    try {
      ...
      if ((!sendTopLevel || !!deferredTopicRootMessageIdForOutput)
        ...
        try {
          ...
          const marker: Record<string, unknown> = { sentAtMs, messageId, turnId: currentTurnId };
...
switch (command) {
  ...
  case 'worktree': {
    const { runRuntimeWorktreeCommand } = await import('./cli/runtime-worktree.js');
    try {
      process.exitCode = await runRuntimeWorktreeCommand(process.argv.slice(3));
    } catch (error) {
      console.error(`运行 worktree 操作失败：${error instanceof Error ? error.message : String(error)}`);
      process.exitCode = 1;
    }
    break;
  }
