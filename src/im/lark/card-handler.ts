...
import { isEditableCardPreviewAction } from './editable-card-preview-handler.js';
...
import { forkWorker, sendWorkerInput, sendWorkerSessionInput, killWorker, closeSession as closeWorkerPoolSession, teardownAuthoritativePersistentBackingBeforeClose, scheduleCardPatch, parkStreamCard, clearUsageLimitState, cardUsageLimit, writableTerminalLinkFor, workerHasInitialized, sessionSupportsWebTerminal, readableTerminalUrlFor, resolvePrivateCardAudience, deliverWriteLinkCard, deliverEphemeralOrReply, CARD_POSTING_SENTINEL, requestSessionRestart, setCodexAppProgressDetailsExpanded, isSessionTransferring, getDaemonStreamingCardUsageSnapshot, withActiveSessionKeyLock } from '../../core/worker-pool.js';
...
export interface CardHandlerDeps {
  ...
  /** 通用可编辑预览动作；服务端记录绑定发起人、源消息、模板和目标群。 */
  editableCardPreviewAction?: (data: CardActionData, larkAppId: string) => Promise<any>;
...
export async function handleCardAction(data: CardActionData, deps: CardHandlerDeps, larkAppId?: string): Promise<any> {
  ...
  if (isEditableCardPreviewAction(value?.action) && larkAppId) {
    if (!deps.editableCardPreviewAction) {
      return { toast: { type: 'error', content: '可编辑预览处理器未启用' } };
    }
    return deps.editableCardPreviewAction(data, larkAppId);
  }
