...
export function codexAppDeveloperInstructions(input: {
  ...
  if (input.locale === 'zh') {
    return [
      ...
      '每条有实质进展的 commentary 末尾追加一行结构化标记（不要放进代码块）：`<!--botmux-progress:{"title":"AI 生成的短任务名","stage":"当前阶段","current":"正在处理","completed":["已完成项"],"next":"下一步","blocker":null,"evidence":[],"delivery":[],"risks":[]}-->`。标记必须是单行有效 JSON；没有真实阻塞时 `blocker` 必须为 `null`。',
