...
export function codexAppDeveloperInstructions(input: {
  ...
  if (input.locale === 'zh') {
    return [
      ...
      '每条有实质进展的 commentary 末尾追加一行结构化标记（不要放进代码块）：`<!--botmux-progress:{"title":"AI 生成的短任务名","stage":"当前阶段","current":"正在处理","completed":["已完成项"],"total":3,"next":"下一步","blocker":null,"evidence":[],"delivery":[],"risks":[],"external":[]}-->`。标记必须是单行有效 JSON；`total` 是可选的正整数总任务数，未知时省略；没有真实阻塞时 `blocker` 必须为 `null`。',
      '涉及 MR、CI 流水线、HAR/组件构建等外部作业时，必须用 `external` 上报每个作业的结构化状态：`"external":[{"label":"MR 8293313","status":"running"},{"label":"HAR 1.0.6-alpha.2 / job 816911007","status":"Failed"}]`；`status` 直接填平台回读到的原始状态（running/queued/success/Failed 等），不要自己润色成结论。进度卡据此严格区分「AI 本轮执行结束」与「外部任务终态」：只要存在未终态或失败的外部作业，就不会展示笼统的「本轮已完成」。',
      '发送最终答案前，先发送最后一条带结构化标记的 commentary，把阶段设为“完成”或“失败”，并填写验证证据、提交或部署交付、剩余风险与 `external` 外部作业终态，供进度卡保留验收摘要。',
  ...
  return [
    ...
    'Append one single-line structured marker to every substantive commentary update, outside code fences: `<!--botmux-progress:{"title":"AI-generated short task title","stage":"current stage","current":"current work","completed":["completed item"],"total":3,"next":"next step","blocker":null,"evidence":[],"delivery":[],"risks":[],"external":[]}-->`. The marker must contain valid JSON; `total` is an optional positive integer and should be omitted when unknown; `blocker` must be null unless human input or external authority is truly required.',
    'Whenever the task involves external jobs such as an MR, CI pipeline, or HAR/component build, report each job through `external` with its structured status: `"external":[{"label":"MR 8293313","status":"running"},{"label":"HAR 1.0.6-alpha.2 / job 816911007","status":"Failed"}]`. Put the platform\'s raw status (running/queued/success/Failed, etc.) in `status`; do not editorialize it into a conclusion. The progress card uses this to strictly separate "the AI turn finished" from "the external task reached a terminal state": as long as any external job is non-terminal or failed, it will not show a blanket "done".',
    'Before the final answer, send one last commentary update with the structured marker, set the stage to completed or failed, and include validation evidence, delivery, remaining risks, and the terminal `external` job statuses so the progress card keeps a closeout summary.',
