...
export const messages: Record<string, string> = {
  ...
  'card.config.quota_off': 'Default: grant card 3 / Oncall unlimited',
  'card.config.quota_value': 'Current: grant cards and Oncall use {quota} messages per person',
  'card.config.quota_edit': 'Set message quota',
  'card.config.quota_legacy_note': '⚠️ Legacy quota {quota}: new grant cards use at most {cardQuota}, while Oncall still uses {quota}. Saving a new quota makes both use the new value.',
  'card.config.quota_legacy_unchanged': 'Current legacy quota kept unchanged',
  'card.config.quota_input_title': '⚙️ {name} · message quota',
  'card.config.quota_input_note': 'Enter a whole number from 1 to 1000. Leave it blank to restore the default: grant cards use 3 messages per person and Oncall is unlimited.',
  ...
  'card.config.quota_saved': 'Message quota set: grant cards and Oncall use {quota} messages per person',
  'card.config.quota_saved_default': 'Default restored: grant cards use 3 messages per person and Oncall is unlimited',
  ...
  'ai.identity.mention_gate': '- **@ hard-gate**: every `botmux send` MUST explicitly pick one or it errors out — `--mention` (name a specific person/bot) / `--mention-back` (@ the triggerer of THIS turn) / `--no-mention` (none). First decide WHETHER to @ by message VALUE: a substantive conclusion the other party should read/confirm/decide on → @ someone; pure record / low-priority progress / short ack → --no-mention; a contentless "got it" is better not sent at all. Then pick HOW by who the recipient is: it is exactly the person/bot that triggered this turn → --mention-back; it is someone else (in a multi-person / multi-bot chat the right recipient is often NOT the triggerer) → --mention to name them. Do not let --no-mention become the default, and do not @ people for nothing.',
  ...
  'card.final_action.submitted': 'Action submitted',
  'card.final_action.already': 'An action from this reply card was already submitted',
  'card.final_action.session_gone': 'The session has ended and cannot continue',
  'card.final_action.need_auth': 'You are not authorized to use this session',
  'card.final_action.worker_busy': 'The session is still running. Wait for the current turn to finish',
  'card.final_action.unsafe': 'This action is not eligible for quick execution; confirm it in text instead',
  'card.final_action.unavailable': 'This version cannot submit the action',
  'card.final_action.failed': 'Failed to submit the action; try again later',
  ...
  'card.ask.flow.waiting_next': 'Answer recorded. Preparing the next question…',
  'card.ask.flow.restoring_previous': 'Restoring the previous question…',
  'card.ask.flow.undo_previous': 'Undo previous answer',
  ...
  'restart.reason': 'Maintenance reason: {reason}',
  'restart.reason_manual': 'The service restart was triggered through the CLI',
  'restart.reason_cli': 'The service restart was triggered through the CLI',
  'restart.reason_ai': 'AI restarted the service with user authorization',
  'restart.reason_dashboard': 'The service restart was triggered through the Dashboard',
  'restart.reason_update': 'A new version was installed; restarting applies the update',
  'restart.reason_rollback': 'A version rollback completed; restarting applies the target version',
  ...
  'restart.turn_progress_stage': 'Stage: {value}',
  'restart.turn_progress_current': 'Current: {value}',
  'restart.turn_progress_completed': 'Completed: {value}',
  'restart.turn_progress_evidence': 'Evidence: {value}',
  'restart.turn_progress_delivery': 'Delivery: {value}',
  'restart.turn_progress_blocker': 'Blocker: {value}',
  'restart.turn_progress_next': 'Next: {value}',
