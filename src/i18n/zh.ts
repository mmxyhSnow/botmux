...
export const messages: Record<string, string> = {
  ...
  'card.config.quota_off': '默认：授权卡 3 条 / Oncall 不限',
  'card.config.quota_value': '当前：授权卡与 Oncall 每人 {quota} 条',
  'card.config.quota_edit': '设置消息额度',
  'card.config.quota_legacy_note': '⚠️ 当前为旧额度 {quota} 条：新授权卡最多按 {cardQuota} 条，Oncall 仍按 {quota} 条。保存新额度后两处将统一。',
  'card.config.quota_legacy_unchanged': '已保留当前旧额度',
  'card.config.quota_input_title': '⚙️ {name} · 消息额度',
  'card.config.quota_input_note': '输入 1–1000 的整数；留空恢复默认：授权卡每人 3 条，Oncall 不限。',
  ...
  'card.config.quota_saved': '已设置消息额度：授权卡与 Oncall 每人 {quota} 条',
  'card.config.quota_saved_default': '已恢复默认：授权卡每人 3 条，Oncall 不限',
  ...
  'card.final_action.submitted': '已提交该操作',
  'card.final_action.already': '这张回复卡已经提交过一个操作',
  'card.final_action.session_gone': '会话已结束，无法继续执行',
  'card.final_action.need_auth': '你没有使用该会话的权限',
  'card.final_action.worker_busy': '会话仍在执行，请等当前回合结束后再点',
  'card.final_action.unsafe': '该操作不符合快捷执行的安全要求，请改用文字确认',
  'card.final_action.unavailable': '当前版本暂不支持回灌该操作',
  'card.final_action.failed': '操作提交失败，请稍后重试',
  ...
  'card.ask.flow.waiting_next': '已记录回答，正在生成下一问…',
  'card.ask.flow.restoring_previous': '正在恢复上一问…',
  'card.ask.flow.undo_previous': '撤销上一问',
  ...
  'card.ask.answerable_turn_callers': '本轮提问对象',
  'card.ask.new_question_notice': '新的 ASK 问题已生成，请查看卡片并作答。',
  ...
  'restart.reason': '维护原因：{reason}',
  'restart.reason_manual': '通过 CLI 触发服务重启',
  'restart.reason_cli': '通过 CLI 触发服务重启',
  'restart.reason_ai': 'AI 按用户授权执行服务重启',
  'restart.reason_dashboard': '通过 Dashboard 触发服务重启',
  'restart.reason_update': '已安装新版本，重启以应用更新',
  'restart.reason_rollback': '已回退版本，重启以应用目标版本',
  ...
  'restart.turn_progress_stage': '阶段：{value}',
  'restart.turn_progress_current': '当前：{value}',
  'restart.turn_progress_completed': '已完成：{value}',
  'restart.turn_progress_evidence': '验证：{value}',
  'restart.turn_progress_delivery': '交付：{value}',
  'restart.turn_progress_blocker': '阻塞：{value}',
  'restart.turn_progress_next': '下一步：{value}',
