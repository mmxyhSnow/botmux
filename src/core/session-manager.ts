...
/**
 * 为重启后仅重挂、尚未收到新消息的 worker 恢复精确 turn 归属。
 * 只有持久化 quote 与当前回复槽位彼此自洽时才复用，避免把历史 turn 当成新授权。
 */
...
/** 重挂始终 resume；存在安全 turn 时一并恢复其精确归属。 */
    ...
    (ds) => {
      ...
      forkWorker(
        ...
        recoverExactNonCodex
          ...
          : restoredWorkerForkOptions(ds.session),
      );
    },
...
export async function ensureTerminalWorkerPort(ds: DaemonSession): Promise<number | undefined> {
  ...
  if (!ds.worker) {
    ...
    if (!forkWorker(ds, '', restoredWorkerForkOptions(ds.session))) {
