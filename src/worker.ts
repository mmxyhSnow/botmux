  ...
  if (kind === 'final') {
    ...
    if (marker.appTurnId) {
      ...
      if (startedAtMs !== undefined) {
        ...
        const gateInput = { markTimeMs: startedAtMs, isLocal: false, finalText: marker.content, turnId: identity.turnId };
        if (shouldSuppressBridgeEmit(gateInput, completedAtMs + 5_001, markers, false)) {
          ...
          const coveringMarker = coveringBridgeSendMarker(gateInput, completedAtMs + 5_001, markers, false);
          notifyExplicitReplyObserved(identity.turnId, coveringMarker
    ...
    if (startedAtMs !== undefined) {
      const markers = readSendMarkers();
      const gateInput = { markTimeMs: startedAtMs, isLocal: false, finalText: marker.content, turnId };
      ...
      if (sentByModel) {
        log(`${cliName()} final_output suppressed (model already called botmux send)`);
        const coveringMarker = coveringBridgeSendMarker(gateInput, completedAtMs + 5_001, markers, false);
        notifyExplicitReplyObserved(turnId, coveringMarker
          ?? explicitReplyMarkerForTurnWindow(gateInput, completedAtMs + 5_001, markers, false));
        if (coveringMarker?.messageId) {
          send({
            type: 'final_output',
