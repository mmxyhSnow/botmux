...
import { bridgePostText, coveringBridgeSendMarker, shouldEmitEmptyCompletedBridgeFallback, shouldEmitFailedBridgeFallback, shouldSuppressBridgeEmit, stripTrailingBridgeSentinelLine, type BridgeSendMarker } from './services/bridge-fallback-gate.js';
  ...
  for (let i = 0; i < ready.length; i++) {
    ...
    if (shouldSuppressBridgeEmit(gateInput, nextBoundaryMs, markers, adoptMode)) {
      ...
      notifyExplicitReplyObserved(
        turn.turnId,
        coveringMarker ?? explicitReplyMarkerForTurnWindow(gateInput, nextBoundaryMs, markers, adoptMode),
      );
...
function markPromptReady(): void {
  ...
  // 立即发送 idle 快照，让飞书卡片反映空闲状态。argv 首轮必须由下方专用
  // 逻辑先发 working，再收敛到 idle/limited；否则会先泄漏错误的空闲状态。
  // 有待处理消息时也跳过：flushPending() 会马上让 CLI 进入忙碌状态。
  if (
    renderer
    && !spawnArgvInitialPromptBusy
