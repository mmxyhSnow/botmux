...
- 主动发给 owner/admin 的维护、告警和权限消息统一使用卡片，不发送文本气泡兜底。除重启通知外，
  同一通知类型持久化一个 messageId，后续状态优先 patch 原卡；只有原卡被撤回或不可更新时才发送
  替代卡，并发首次通知必须通过跨进程锁收敛为一张。每次真实的主动重启都必须新发一张维护卡，
  让本次重启结果出现在聊天最新位置；不得更新历史重启卡。
...
- Skill 对齐、daemon 离线、主机过载和权限健康各自使用独立卡片槽位；重启通知逐次新发；发版生命周期
  继续复用 HEAD 绑定的发版卡，不能把不同授权边界混到一张卡里。
